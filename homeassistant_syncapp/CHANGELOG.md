...
- Add an explicit disposable-HAOS backup archive probe that streams the fresh Supervisor backup download to exclusive no-follow temporary storage under a hard byte ceiling, structurally traverses the outer tar and embedded Home Assistant tar without extracting configuration, rejects unsafe member paths, cleans partial downloads on failure, and blocks optional Core restart when archive proof fails.
...
- Pin every version-2 rollback snapshot file by SHA-256 and re-verify those bytes before apply and before/during rollback restoration so a damaged snapshot cannot be restored into `/homeassistant`.
