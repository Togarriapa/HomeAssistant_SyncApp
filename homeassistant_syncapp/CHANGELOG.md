...
- Bind an existing `/data/repository` clone to its original `origin`; changing the configured managed repository now fails closed instead of silently rewriting `origin` and reusing unrelated Git/manifest state.
