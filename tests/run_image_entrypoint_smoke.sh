...
# Exercise the image's real CMD and /run.sh shebang. Only the final
# /usr/bin/python3 executable is bind-mounted with a recorder so the service
# does not contact Supervisor or enter the synchronization loop.
docker run --rm \
...
# /run.sh must pin the exact interpreter argv before the Python bootstrap gets
# a chance to construct the smaller long-lived service environment.
...
# The shell boundary itself must remove Python startup controls and ELF loader
# controls before replacing itself with the bootstrap interpreter.
for forbidden in \
...
# These values intentionally remain until process_bootstrap.py constructs the
# strict service allowlist; their presence here proves this smoke is observing
# the pre-bootstrap exec boundary rather than accidentally bypassing it.
grep -Fx 'HTTPS_PROXY=http://untrusted.invalid:8080' "${EVIDENCE_PATH}"
grep -Fx 'GIT_DIR=/untrusted/git' "${EVIDENCE_PATH}"

# The Supervisor credential must remain environment-only, never part of argv.
