...
The persisted managed-path manifest under `/data` is treated as safety-critical state, not a best-effort cache. If it is malformed, has the wrong JSON shape, or names a blocked/absolute/traversal path, synchronization fails closed before local mirroring. Managed paths are revalidated immediately before deletion so damaged state cannot turn the isolated repository cleanup path into a filesystem traversal. Manifest updates are written to a temporary file, flushed with `fsync`, atomically replaced, and followed by a parent-directory `fsync`; if durability cannot be proven, SyncApp reports failure so verified transaction evidence can be preserved and retried rather than treating bookkeeping as complete.
